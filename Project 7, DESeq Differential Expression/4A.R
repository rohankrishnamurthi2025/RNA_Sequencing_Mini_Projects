...
#yeast_count_table_replacements <- read.table("count_table_replacements.txt")

# first get vector of column names
samplename_1 <- colnames(yeast_count_table_original)
...
DDS_yeast_1 <- DESeqDataSetFromMatrix(countData = yeast_count_table_original,
...
# Heatmap 2
vsd_1.2 <- vst(DDS_yeast_1, blind=FALSE)
select_1.2 <- order(rowMeans(counts(DDS_yeast_1, normalized=TRUE)),
                  decreasing=TRUE)[1:30]
df_1.2 <- as.data.frame(colData(DDS_yeast_1)[,c("condition","sample")])
pheatmap(assay(vsd_1.2)[select_1.2,], cluster_rows=TRUE, show_rownames=TRUE,
         cluster_cols=TRUE, annotation_col = df_1.2)

# Heatmap 3
vsd_1.3 <- vst(DDS_yeast_1, blind=FALSE)
select_1.3 <- order(rowVars(counts(DDS_yeast_1, normalized=TRUE)),
                    decreasing=TRUE)[1:30]
df_1.3 <- as.data.frame(colData(DDS_yeast_1)[,c("condition","sample")])
pheatmap(assay(vsd_1.3)[select_1.3,], cluster_rows=TRUE, show_rownames=TRUE,
         cluster_cols=TRUE, annotation_col = df_1.3)

# PCA 2
pca_data_2 <- plotPCA(vsd_1, intgroup=c("condition", "sample"),
